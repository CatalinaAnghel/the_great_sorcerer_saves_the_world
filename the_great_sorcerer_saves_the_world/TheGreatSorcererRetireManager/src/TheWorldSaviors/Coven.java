...
public class Coven extends Thread{
	 ...
	 * every demon when he asks to be registered into the coven or by every demon who wants to leave the coven 
	 * after he is retired. 
	 */
	...
	public synchronized void receiveIngredient(List<String> ingredients) {
		...
		if(!this.witches.isEmpty() && !this.ingredients.isEmpty()) {
			turn = true;
			notifyAll();
		} 
	...
	/**
	 * This is the method used by a demon when he asks to be retired from the coven.
	 * @param demon Demon - the demon that asks to be removed from the coven's list of demons.
	 */
	public synchronized void askToBeRemovedFromCoven(Demon demon) {
		try {
			queue.acquire();
			demonsSem.acquire();
			queue.release();
			demons.remove(demon);
			System.out.println("The demon no." + demon.demonId +"from the coven no." + demon.coven.getCovenId() + " was removed from its coven.");
		} catch (InterruptedException e) {
			e.printStackTrace();
		}finally {
			demonsSem.release();
		...
		try {
			roomsSem.acquire();
			this.rooms[demon.row][demon.column] = null;
		} catch (InterruptedException e) {
			e.printStackTrace();
		}finally{
			roomsSem.release();

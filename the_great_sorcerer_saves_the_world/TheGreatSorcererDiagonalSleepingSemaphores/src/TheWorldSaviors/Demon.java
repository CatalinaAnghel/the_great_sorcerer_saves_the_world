...
public class Demon extends Thread {
	...
	/**
	 * This variable shows if the current demon is asleep. A demon falls asleep if his room is on the diagonal
	 * of the rooms matric(row = column)
	 */
	...
	public synchronized Boolean moveUp() {
		...
		// compute the next room position
		this.row--;
		if(row < 0) {
			// he kicked a wall and he will go back
			row++;
			this.noKickedWalls++;
			...
			success = true;
			if(row == column) {
				// if the demon's current room is on the matrix's diagonal, the demon will sleep
				goToSleep();
			}
		}
	...
	/**
	 * This method is called when a demon waits to wake up. He will wake up just when all the coven's
	 * demons are asleep.
	 */
	public void wakeUp() {
		if(asleep) {
			// if the demons is asleep
			try {
				// acquire the common semaphore
				coven.barrier.acquire();
				// notify the coven that the demon is asleep
				coven.demonIsSleeping();
				// release the semaphore and let another demon to register as an asleep demon
				coven.barrier.release();
				
				if(coven.sleepingDemons.equals(coven.capacity/2)) {
					// if all the coven's demons are asleep, release the barrier
					coven.semaphores.get(this).release();
				}
				// the demon will acquire his semaphore in order to leave the barrier
				coven.semaphores.get(this).acquire();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}finally {
				// release his semaphore
				coven.semaphores.get(this).release();
			}
		}
	}
	...
	/**
	 * This is the method used by a demon when he falls asleep, when his room's row is equal with the room's
	 * column
	 */
	public void goToSleep() {
		this.asleep = true;
		System.out.println("The demon " + demonId + "from the coven no." + coven.covenId +" is sleeping.");
		// call the method wakeUp because he needs to know when to wake up
		wakeUp();
	}

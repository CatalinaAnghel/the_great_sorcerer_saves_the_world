...
public class Coven extends Thread{
	...
	public void run() {
		while(true) {
			...
			// Release the semaphore of demons
			demonsSem.release();
		}
	}
	...
	public synchronized void changeRoom(Demon demon, Integer oldRow, Integer oldColumn, Integer row, Integer column) {
		try {
			// acquire the rooms semaphore to access the shared resource
			roomsSem.acquire();
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
	...
	public synchronized void visit(List<String> wIngredients, Witch witch) {
		...
		if(!this.ingredients.isEmpty()) {
			// if there are ingredients left, the witch will try to get access to the shared resource
			try {
				ingredientsSem.acquire();
			} catch (InterruptedException e) {
				e.printStackTrace();
			...
			// remove the ingredients taken by the witch from the list of ingredients
			this.ingredients.removeAll(receivedIngredients);
			// release the semaphore
			ingredientsSem.release();
			System.out.println("Coven: ingredients: " + this.ingredients);
			// take the ingredients from the local variable called receivedIngredients
			witches.get(witches.indexOf(witch)).takeIngredients(receivedIngredients);
		}
		// ask for access to the vector of attackers
		try {
				undeadSem.acquire();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
		// if the coven is attacked, confront an undead and start the fight
		if(!attackers.isEmpty()) {
			attackers.remove(attackers.size() - 1);
			fight(witch);
		}
		// release the semaphore for undeads
		undeadSem.release();
			
		// lock the vector of visiting witches and remove the current witch and then unlock it. 
		lock.lock();
		witches.remove(witch);
		lock.unlock();
		
		// if there are not ingredients left or all the witches left the coven, turn = false and the demons
		// can send their ingredients
	...
	public synchronized void fight(Witch witch) {
		try {
			// the witch asks for help from the Grand Sorcerer
		...
		if(witch.getPotions().size() > 0) {
			// if the witch has the potions, she will use them and the undead will be scared.
			...
			// the witch does not have potions and 10% of ingredients are gone
			...
			try {
				// acquire access to the list of ingredients
				ingredientsSem.acquire();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			// remove 10% of the ingredients
	...
	public synchronized void attack(Undead undead) {
		try {
			// acquire the semaphore for the undeads
			undeadSem.acquire();
		} catch (InterruptedException e1) {
			e1.printStackTrace();
		}
		// add the undead who attacks the coven
		...
		if(!witches.isEmpty()) {
			// if there is a witch in the coven, she will try to defend it.
			...
			// if the coven is not visited by witches
			Integer iterator = 0;
			Random random = new Random();
			// generate the random number of demons that will be scared
			Integer noRetiredDemons = random.nextInt(5)+5;
			try {
				// wait in line to be serviced
				queue.acquire();
				// acquire the semaphore for the demons list
				demonsSem.acquire();
				// let the next one in line to be serviced
				queue.release();
			} catch (InterruptedException e1) {
				e1.printStackTrace();
			}
			if(demons.size() <= noRetiredDemons || !demons.isEmpty()) {
				// if there are not enough demons, all of them will be scared
				...
				// there are not demons at all
				System.out.println("The coven did not have demons. But the ingredients are gone");
			}
			// release the semaphore for the list of demons
			demonsSem.release();
			try {
				// acquire the semaphore for the list of ingredients
				ingredientsSem.acquire();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			...
			// release the semaphore for the list of ingredients
			ingredientsSem.release();
			try {
				// acquire the semaphore for the undeads
				undeadSem.acquire();
